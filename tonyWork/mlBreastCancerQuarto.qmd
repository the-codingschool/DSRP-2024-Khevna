...
write.csv(final_accuracies_standard, "final_accuracies_standard.csv", row.names = FALSE)
```
...
From what we see, we can decide that we should use the subset and not the features or all the values because we have removed the highly correlated variables and the features one only show which are most important but don't say anything about it being completely adequate to predict or as adequate as the subset. We should also use our standardized data. Now, let's find which model is best with these constraints.

```{r}
final_accuracies_standard_subset <- final_accuracies_standard %>% filter(Metric == "Sub")

ggplot(final_accuracies_standard_subset,aes(x = reorder(Model, Accuracy, FUN = median),y=Accuracy,group=Model))+geom_boxplot()
```
...
auc2 <- auc(roc_curve2)
cat("AUC for model2:", round(auc2, 2), "\n")
```
