...
To make sure there not any highly correlated variables, we can calculate the correlation between every numerical variable and then plot those that are above 0.9.
...
As seen in the graph, there are none.
...
We see that ncc is one of the better models as it is at or very close to the best in terms of both accuracy, precision. It also has the lowest false negative rate, which is very good. We can now look at this best model with cross validation, which we didn't explicitly include earlier.
...
Let's also generate an ROC curve and it's corresponding AUC.
...
Now let's compare the cross-validated model to the original by repeating this 200 times.
