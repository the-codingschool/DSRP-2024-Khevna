...
```{r}
column_names <- c("Model", "Metric", "Accuracy") 
accuracy_dataframe <- data.frame(matrix(ncol = length(column_names), nrow = 0))
colnames(accuracy_dataframe) <- column_names
```
...
rbind(accuracy_dataframe,accuracies)
```
