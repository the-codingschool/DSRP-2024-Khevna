...
TTest between metrics for non-normalized

```{r}
dataset <- final_accuracies
metrics <- unique(dataset$Metric)
pval_matrix <- matrix(NA,nrow=length(metrics),ncol=length(metrics),dimnames=list(metrics,metrics))
for(i in 1:length(metrics)){
  for(j in 1:length(metrics)) {
    arm1 <- dataset %>% filter(Metric == metrics[[i]]) %>% pull(Accuracy)
    arm2 <- dataset %>% filter(Metric == metrics[[j]]) %>% pull(Accuracy)
    ttest <- t.test(arm1,arm2)
    pval_matrix[i,j] = ttest$p.value
  }
}

pval_df <- as.data.frame(pval_matrix)
pval_df <- pval_df %>% select(everything())
pval_df < 0.05 # True means significant
...
pval_matrix <- matrix(NA,nrow=length(models),ncol=length(models),dimnames=list(models,models))
for(i in 1:length(models)){
  for(j in 1:length(models)) {
    arm1 <- dataset %>% filter(Model == models[[i]]) %>% pull(Accuracy)
    arm2 <- dataset %>% filter(Model == models[[j]]) %>% pull(Accuracy)
    ttest <- t.test(arm1,arm2)
    pval_matrix[i,j] = ttest$p.value
  }
}
...
```{r}
dataset <- final_accuracies_standard
metrics <- unique(dataset$Metric)
pval_matrix <- matrix(NA,nrow=length(metrics),ncol=length(metrics),dimnames=list(metrics,metrics))
for(i in 1:length(metrics)){
  for(j in 1:length(metrics)) {
